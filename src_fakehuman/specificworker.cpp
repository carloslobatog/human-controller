...
void SpecificWorker::includeInAGM()
{
	...
	personMesh->setAttribute("path", "/home/lobato/robocomp/components/robocomp-shelly/models/human01.3ds");
...
*/
	...
	//PRUEBA
	connect(prueba,SIGNAL(pressed()),this,SLOT(pruebaP()));
	connect(prueba,SIGNAL(released()),this,SLOT(pruebaR()));
...
//PRUEBA()
void SpecificWorker::pruebaP()
{
	tbutton.prueba = true;
}

void SpecificWorker::pruebaR()
{
	tbutton.prueba = false;
}
...
/*	 	//PRUEBA
	else if (tbutton.prueba==true)
	{
		cout<<"prueba is ok"<<endl;
...
void SpecificWorker::wander(){

	RoboCompInnerModelManager::coord3D coordInItem;
  	RoboCompInnerModelManager::coord3D coordInBase;

	if (tbutton.prueba == true){

		coordInItem.y = 0;
		
		// Move direction from a random number
		int direction;
		srand((unsigned) time(NULL));
		direction = (rand() % 3) +1; // random number between 1 and 4
